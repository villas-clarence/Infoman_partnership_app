...
$stmt = $conn->prepare("INSERT INTO registrants (
    full_name, email, mobile, company_name, role, contact_method,
    certificate_name, dashboard_pref, gift_pref, notification_pref,
    zoom_call, agreement, created_at
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
...
if (!$pay_stmt) {
    die("Prepare failed for payments: " . $conn->error);
}
